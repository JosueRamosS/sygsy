...
export const ProfessorsPage = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setIsLoading(true);
        try {
            // Create user as PROFESSOR
            // If career is empty string, send null (or handle in backend, but usually null is better)
            const payload = {
                ...formData,
                role: 'PROFESSOR',
    ...
    const careerOptions = [
        { value: '', label: 'Sin Carrera (General)' },
        ...careers.map(c => ({ value: c.name, label: c.name }))
    ];
        ...
        <div className="min-h-screen bg-neo-bg p-8 font-sans">
            <div className="max-w-6xl mx-auto">
                <div className="flex justify-between items-center mb-8">
                    <div>
                        <h1 className="text-3xl font-black uppercase tracking-tighter">Gestión de Docentes</h1>
                        <p className="text-gray-600 font-medium">Administra la plana docente.</p>
                    </div>
                        ...
                        <table className="w-full text-left">
                            <thead className="bg-neo-yellow border-b-4 border-black">
                                <tr>
                                    ...
                                    <th className="p-4 font-black uppercase">Carrera Principal</th>
                            ...
                            <tbody className="divide-y-2 divide-black">
                                {users.map((user) => (
                                    <tr key={user.id} className="hover:bg-gray-50">
                                        ...
                                        <td className="p-4">
                                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-black uppercase border border-black bg-neo-green text-black">
                                                <Shield size={12} />
                ...
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Nuevo Docente">
